...
- **릴리즈 전 자동 자(ruler) 하나 이상 권장** — 화면 순회는 화면 밖 회귀를 못 잡는다
  (KOF R-2 v0.2 소리 전멸이 화면 검증을 전부 통과했다). 검증된 자 두 개:
  ①**소리 계측**(하네스에서 원본 vs 패치본 음량 숫자 비교 — 이식소 ngprun 에 있음)
  ②**오프셋 표 불변식**(문자열 표의 모든 항목이 여전히 문자열 시작을 가리키는가 —
  이식소 kof_tables.py, 롬만 바꾸면 재사용. 모드바이트 집합은 게임마다 재측정).
  상세는 thinkbox traps.md 「소리는 소리로 재라」 절.
...
## 받기 UX (2026-09-03 추가 — 「다운로드 링크가 안 보인다」 제보)

- **Assets 에는 패치 파일만 둔다** (.ips/.bps/README.txt). 스크린샷 PNG 를 자산으로
  올리면 받을 파일이 묻힌다 — 스샷은 레포 `images/<태그>/` 에 커밋하고 본문에서
  raw.githubusercontent 로 링크한다.
- **본문 맨 위는 「## ⬇ 받기」 배너** — 주 파일(파생 접미사 없는 .ips/.bps)의 직링과
  크기. 파생판(_allcards·_to_·_old)은 배너에서 빼고 Assets 안내 한 줄로 갈음.
- 현행 릴리즈 전부에 2026-09-03 일괄 적용됨 (도구: 본부 scratchpad/krux.py).
