...
IPS 를 올린 것만으로는 앱에 **아직 안 간다.** 앱은 `pocketcore` 태그의 `patches.json` 색인을
보는데, 이 색인은 SP 방의 배포 스크립트가 릴리즈 실태를 훑어 재생성한다.
...
`pocketcore`(앱 APK·version.json·patches.json·cores.json), `core-svc`·`core-ss2`(코어 .so),
`ss2-voice`(더빙 음성팩)는 앱 자동 업데이트가 보는 고정 태그다. 자산 추가·삭제·개명 금지 —
SP 방 스크립트(pub_pocketcore.py / pub_content.py)만 만진다.
